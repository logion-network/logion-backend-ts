...
describe("AlchemyService", () => {
    ...
    it("returns some owner with expected input", async () => {
        ...
        expect(owners[0].equals(expectedOwners[0])).toBeTrue();
    })
...
function buildAlchemyService(): AlchemyService {
    ...
    alchemyMock.setup(instance => instance.nft.getOwnersForNft(expectedContractAddress, expectedTokenId)).returnsAsync({ owners: expectedOwners.map(owner => owner.address) });
...
const expectedOwners = [ ValidAccountId.ethereum("0xa6db31d1aee06a3ad7e4e56de3775e80d2f5ea84") ];
